...
DISPATCH_TASKS: dict[str, asyncio.Task] = {}
...
def favorite_priority_window(scheduled_at: datetime) -> tuple[int, int]:
    """Возвращает (эксклюзивное ожидание, повторное уведомление) в секундах.

    exclusive=0 означает: сразу публиковать всем подходящим водителям,
    но любимого водителя уведомить отдельно.
    """
    seconds_to_pickup = max(
        0,
        int((scheduled_at.astimezone(MSK) - now_msk()).total_seconds()),
    )

    if seconds_to_pickup > 3 * 3600:
        return 60 * 60, 30 * 60
    if seconds_to_pickup > 60 * 60:
        return 30 * 60, 15 * 60
    if seconds_to_pickup >= 30 * 60:
        return 5 * 60, 3 * 60
    if seconds_to_pickup >= 15 * 60:
        return 2 * 60, 1 * 60
    return 0, 0


def favorite_driver_for_order(
    context: ContextTypes.DEFAULT_TYPE,
    client_id: int,
    order: dict[str, Any],
) -> Optional[dict[str, Any]]:
    ensure_storage(context)
    client = context.bot_data["clients"].get(str(client_id), {})
    favorite_ids = client.get("favorite_driver_ids") or []
...
async def publish_order_to_driver_group(
    context: ContextTypes.DEFAULT_TYPE,
    order_id: str,
    order: dict[str, Any],
) -> None:
    ...
    order["orders_chat_id"] = target_chat_id
    order["group_message_id"] = group_message.message_id
    order["published_to_group_at"] = now_iso()
    if order.get("status") in {"favorite_pending", "open"}:
        order["status"] = "open"
    track_message(context, order_id, target_chat_id, group_message.message_id)


async def publish_order_to_driver_group_app(
    ...
    target_chat_id = ORDERS_CHAT_ID
    group_message = await application.bot.send_message(
        target_chat_id,
        order_public_text(order_id, order),
        parse_mode="HTML",
        reply_markup=keyboard,
    ...
    registry = application.bot_data.setdefault("message_registry", {}).setdefault(order_id, [])
    item = (int(target_chat_id), int(group_message.message_id))
    if item not in registry:
        registry.append(item)


async def notify_favorite_driver(
    context: ContextTypes.DEFAULT_TYPE,
    order_id: str,
    order: dict[str, Any],
    driver: dict[str, Any],
    ...
    deadline = now_msk() + timedelta(seconds=priority_seconds)
    text = (
        "⭐ Заказ от постоянного клиента\n\n"
        f"{order_public_text(order_id, order)}\n\n"
    )
    if priority_seconds:
        text += f"Приоритет действует до {deadline.strftime('%H:%M')}."
    else:
        text += "Заказ срочный и одновременно опубликован другим водителям."

    keyboard = InlineKeyboardMarkup(
        [[
            InlineKeyboardButton(
                "✅ Принять",
                callback_data=f"favtake_{order_id}",
            ),
            InlineKeyboardButton(
                "❌ Не смогу",
                callback_data=f"favdecline_{order_id}",
            ),
        ]]
    )
    ...
    order["favorite_message_id"] = message.message_id


async def favorite_dispatch_timer(
    application: Application,
    order_id: str,
) -> None:
    try:
        order = application.bot_data.get("orders", {}).get(order_id)
        if not order or order.get("status") != "favorite_pending":
            return

        reminder_at = parse_iso(order.get("favorite_reminder_at"))
        release_at = parse_iso(order.get("favorite_priority_until"))

        if reminder_at:
            wait = (reminder_at - now_msk()).total_seconds()
            if wait > 0:
                await asyncio.sleep(wait)

            order = application.bot_data.get("orders", {}).get(order_id)
            if order and order.get("status") == "favorite_pending":
                try:
                    await application.bot.send_message(
                        int(order["favorite_driver_id"]),
                        (
                            f"⏳ Заказ №{order_id} ещё доступен.\n"
                            "Приоритет скоро закончится."
                        ),
                    )
                except TelegramError:
                    logger.exception("Не удалось отправить повторное уведомление любимому водителю")

        if release_at:
            wait = (release_at - now_msk()).total_seconds()
            if wait > 0:
                await asyncio.sleep(wait)

        order = application.bot_data.get("orders", {}).get(order_id)
        if order and order.get("status") == "favorite_pending":
            await publish_order_to_driver_group_app(
                application,
                order_id,
                order,
            )
            try:
                await application.bot.send_message(
                    int(order["client_id"]),
                    "Любимый водитель не принял заказ. Поиск расширен на других подходящих водителей.",
                )
            except TelegramError:
                logger.exception("Не удалось уведомить клиента о расширении поиска")
    except asyncio.CancelledError:
        raise
    except Exception:
        logger.exception("Ошибка таймера приоритетного распределения")
    finally:
        DISPATCH_TASKS.pop(order_id, None)


def schedule_favorite_dispatch(
    context: ContextTypes.DEFAULT_TYPE,
    order_id: str,
) -> None:
    old = DISPATCH_TASKS.pop(order_id, None)
    if old:
        old.cancel()
    DISPATCH_TASKS[order_id] = context.application.create_task(
        favorite_dispatch_timer(context.application, order_id)
    )
...
async def publish_order(
    ...
    favorite = None if order.get("special_request") else favorite_driver_for_order(
        context,
        client_id,
        order,
    )
    scheduled_at = parse_iso(order.get("scheduled_at")) or now_msk()
    priority_seconds, reminder_seconds = favorite_priority_window(scheduled_at)

    if favorite:
        order["favorite_driver_id"] = int(favorite["user_id"])
        await notify_favorite_driver(
            context,
            order_id,
            order,
            favorite,
        ...
        if priority_seconds > 0:
            order["status"] = "favorite_pending"
            order["favorite_priority_until"] = (
                now_msk() + timedelta(seconds=priority_seconds)
            ).isoformat()
            order["favorite_reminder_at"] = (
                now_msk() + timedelta(seconds=reminder_seconds)
            ).isoformat()
            schedule_favorite_dispatch(context, order_id)
        else:
            await publish_order_to_driver_group(context, order_id, order)
    else:
        await publish_order_to_driver_group(context, order_id, order)

    client_notice = (
        f"⭐ Заказ №{order_id} сначала отправлен любимому водителю."
        if order.get("status") == "favorite_pending"
        else f"✅ Заказ №{order_id} отправлен водителям."
    )
    await query.edit_message_text(
        client_notice,
...
async def favorite_take_order(
    ...
    order_id = query.data.removeprefix("favtake_")
    order = context.bot_data["orders"].get(order_id)
    driver = context.bot_data["drivers"].get(str(query.from_user.id))

    if not order or order.get("status") != "favorite_pending":
        await query.answer("Приоритет уже закончился или заказ недоступен.", show_alert=True)
        return
    if order.get("favorite_driver_id") != query.from_user.id:
        await query.answer("Этот заказ предназначен другому водителю.", show_alert=True)
        return
    if not driver or driver.get("status") != "approved":
        await query.answer("Ваш доступ к заказам закрыт.", show_alert=True)
        return
    ...
    task = DISPATCH_TASKS.pop(order_id, None)
    if task:
        task.cancel()
    ...
    await query.edit_message_text(f"✅ Вы приняли заказ №{order_id}.")


async def favorite_decline_order(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> None:
    ensure_storage(context)
    ...
    order_id = query.data.removeprefix("favdecline_")
    order = context.bot_data["orders"].get(order_id)
    ...
    await publish_order_to_driver_group(context, order_id, order)
    await query.edit_message_text(
        f"Вы отказались от приоритетного заказа №{order_id}."
    )
    await context.bot.send_message(
        int(order["client_id"]),
        "Любимый водитель не сможет выполнить заказ. Поиск расширен на других подходящих водителей.",
    )
...
async def add_favorite_driver(
    ...
    order_id = query.data.removeprefix("addfavorite_")
    order = context.bot_data["orders"].get(order_id)
    ...
    client = context.bot_data["clients"].setdefault(str(query.from_user.id), {})
    favorites = client.setdefault("favorite_driver_ids", [])
    if driver_id not in favorites:
        favorites.insert(0, driver_id)

    await query.edit_message_reply_markup(reply_markup=None)
    await query.answer("Водитель добавлен в любимые.", show_alert=True)
...
async def cancel_open_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if order.get("status") not in {"open", "favorite_pending"}:
        await query.answer(
    ...
    if group_message_id:
        try:
            await context.bot.delete_message(
                chat_id=order.get("orders_chat_id") or ORDERS_CHAT_ID,
...
async def finish_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_message(
        ...
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton(
                "⭐ Добавить водителя в любимые",
                callback_data=f"addfavorite_{order_id}",
            )]]
        ),
    )
    if order["driver_id"] != order["client_id"]:
        await context.bot.send_message(
            order["driver_id"],
            summary,
            reply_markup=MAIN_KB,
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if order_id:
        order = context.bot_data["orders"].get(order_id)
        if order and order.get("status") in {"open", "favorite_pending"}:
            order["status"] = "cancelled"
            order["cancelled_at"] = now_iso()
...
async def restore_dispatch_tasks(application: Application) -> None:
    """Восстанавливает таймеры приоритетных заказов после перезапуска."""
    for order_id, order in list(application.bot_data.get("orders", {}).items()):
        if order.get("status") != "favorite_pending":
            continue

        release_at = parse_iso(order.get("favorite_priority_until"))
        if not release_at or release_at <= now_msk():
            await publish_order_to_driver_group_app(application, order_id, order)
            try:
                await application.bot.send_message(
                    int(order["client_id"]),
                    "Любимый водитель не принял заказ. Поиск расширен на других подходящих водителей.",
                ...
                logger.exception("Не удалось уведомить клиента после перезапуска")
            continue

        DISPATCH_TASKS[order_id] = application.create_task(
            favorite_dispatch_timer(application, order_id)
        )
...
def main() -> None:
    ...
    application = (
        ...
        .post_init(restore_dispatch_tasks)
    ...
    registration_conversation = ConversationHandler(
        ...
        fallbacks=[CommandHandler("cancel", cancel)],
        allow_reentry=True,
        name="driver_registration_v5_3_clean",
        persistent=True,
    )
    ...
    application.add_handler(
        CallbackQueryHandler(
            favorite_take_order,
            pattern=r"^favtake_[A-F0-9]{8}$",
        )
    )
    application.add_handler(
        CallbackQueryHandler(
            favorite_decline_order,
            pattern=r"^favdecline_[A-F0-9]{8}$",
        )
    )
    application.add_handler(
        CallbackQueryHandler(
            add_favorite_driver,
            pattern=r"^addfavorite_[A-F0-9]{8}$",
        )
    )
    application.add_handler(
