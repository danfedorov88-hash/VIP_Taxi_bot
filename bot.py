...
async def drivers_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for uid, driver in drivers.items():
        ...
        if status == "approved":
            buttons.append(
                InlineKeyboardButton("🚫 Заблокировать", callback_data=f"blockdrv_{uid}")
            )
        elif status == "blocked":
            buttons.append(
                InlineKeyboardButton("♻️ Разблокировать", callback_data=f"unblockdrv_{uid}")
            )

        buttons.append(
            InlineKeyboardButton("🗑 Удалить", callback_data=f"deletedrv_{uid}")
        )
...
async def take_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if (
        not order.get("special_request")
        and order.get("car_class") not in {"Неважно", driver.get("car_class")}
    ):
        await query.answer("Класс автомобиля не подходит.", show_alert=True)
        return

    driver_active = context.bot_data["active_by_user"].get(str(query.from_user.id))
    if driver_active:
    ...
    try:
        ...
        logger.exception("Не удалось убрать кнопку принятия заказа")
    ...
    await context.bot.send_message(order["client_id"], vehicle_text)

    car_photos = driver.get("car_photos") or []
    if car_photos:
        for photo_id in car_photos:
            try:
                await context.bot.send_photo(order["client_id"], photo=photo_id)
            except TelegramError:
                logger.exception("Не удалось отправить фото автомобиля")
    else:
        await context.bot.send_message(
            order["client_id"],
            "⚠️ В анкете водителя нет сохранённых фотографий автомобиля.",
        )
...
async def finish_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    context.bot_data["active_by_user"].pop(str(order["driver_id"]), None)
