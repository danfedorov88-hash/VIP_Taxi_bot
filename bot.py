...
async def offer_special_order(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    offer_keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton("📸 Посмотреть фото", callback_data=f"offerphotos_{order_id}_{uid}")],
            [
                InlineKeyboardButton("✅ Выбрать машину", callback_data=f"chooseoffer_{order_id}_{uid}"),
                InlineKeyboardButton("❌ Не подходит", callback_data=f"rejectoffer_{order_id}_{uid}"),
            ],
        ]
    )
    await tracked_send_message(
        ...
        order["client_id"],
        (
            f"🚘 Предложение по особому запросу №{order_id}\n"
            f"Автомобиль: {driver.get('car', '—')} {driver.get('year', '')}\n"
            f"Госномер: {driver.get('plate', '—')}\n"
            f"Класс: {driver.get('car_class', '—')}\n\n"
            "Можно посмотреть фото, выбрать эту машину или отклонить предложение."
        ),
...
async def choose_special_offer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await assign_driver_to_order(context, order_id, order, int(driver_uid))
    await query.edit_message_text(
...
async def reject_special_offer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text(
        "❌ Машина отклонена. Можно дождаться другого предложения."
    )
